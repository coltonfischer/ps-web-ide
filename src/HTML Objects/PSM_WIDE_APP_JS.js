function updateEditorValue(val, visible) {
  ...
  if (w2ui.layout_main_tabs.get(w2ui.layout_main_tabs.active).prettyKey.startsWith('PACKAGE') && w2ui.layout_main_tabs.get(w2ui.layout_main_tabs.active).pc.includes('import PSM_RUN_PC:IRunner')) {
    w2ui.toolbar.show('runPC');
  } else {
    w2ui.toolbar.hide('runPC');
  }
}
...
function runPC(url) {
  if (!w2ui.layout_main_tabs.get(w2ui.layout_main_tabs.active)) {
    alert('You must have an active tab open to run.');
  }
  var popup = {};
  popup.url = url + '?key=' + w2ui.layout_main_tabs.get(w2ui.layout_main_tabs.active).id;
  popup.showMax = true;
  w2popup.load(popup);
}
    ...
    if (xmlhttp.readyState == XMLHttpRequest.DONE) {
      ...
      if (xmlhttp.status == 200 && xmlhttp.getResponseHeader("Custom-Status") == 200) {
        if (obj.useWorker) {
          ...
          ace.config.set("workerPath", obj.workerUrl + '?');
        } else {
          editor.session.setOption("useWorker", false);
        }
        if (ace.config.all().$moduleUrls[obj.aceMode] != obj.modeUrl) {
          ace.config.setModuleUrl(obj.aceMode, obj.modeUrl);
        }
...
function populateSidebar(data, init) {
  for (var key in data.progkeys) {
    ...
    lastnode.modeUrl = ob[data.objectType].sidebarNode.childInfo.modeUrl;
    lastnode.useWorker = ob[data.objectType].sidebarNode.childInfo.useWorker;
    lastnode.workerUrl = ob[data.objectType].sidebarNode.childInfo.workerUrl;
    ...
    },
    {
      ...
      tabs: {
        ...
        onClick: function (event) {
          ...
          if (w2ui["sidebar"].get(event.target).useWorker) {
            editor.session.setOption("useWorker", true);
            ace.config.set("workerPath", w2ui["sidebar"].get(event.target).workerUrl + '?');
          } else {
            editor.session.setOption("useWorker", false);
          }
          if (ace.config.all().$moduleUrls[w2ui["sidebar"].get(event.target).aceMode] != w2ui["sidebar"].get(event.target).modeUrl) {
            ace.config.setModuleUrl(w2ui["sidebar"].get(event.target).aceMode, w2ui["sidebar"].get(event.target).modeUrl);
          }
...
w2ui["layout"].content("top", $("#toolbar").w2toolbar({
  ...
  onClick: function (event) {
    ...
    if (event.item.id === "runPC") {
      runPC(event.item.url);
    }
  }
